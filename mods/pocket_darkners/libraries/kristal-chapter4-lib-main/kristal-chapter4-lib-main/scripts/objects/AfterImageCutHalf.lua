local AfterImageCutHalf, super = Class(Object)

function AfterImageCutHalf:init(texture, after)
    super.init(self, x, y)
...
function AfterImageCutHalf:onAdd(parent)
    super.onAdd(parent)

    self.start_color = self.color
end

function AfterImageCutHalf:update()
...
function AfterImageCutHalf:draw()
    if self.flash then
